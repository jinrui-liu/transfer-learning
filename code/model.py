...
# A note on how models read in data:
# Our mental model of the dataset is often as a matrix: rows correspond to samples, columns correspond to drugs, and the entry (i, j) coresponds to sample i and drug j. 
# However, we don't pass in the data as a matrix. Instead, we pass in a list of sample_ids "s_idx", a list of drug_ids "d_idx", and a list of observations "obs". Then 
# obs[i] corresponds to sample s_idx[i] and drug d_idx[i]. 

# The transfer model trains on both source and target data. We call the target data used for training "target_train" to distinguish from the heldout target data.
...
# d_idx2: lsit of drug_ids for target_train
...
def transfer_model(n_samp, n_drug, s_idx1, d_idx1, s_idx2, d_idx2, obs1=None, n_obs1=None, obs2=None, n_obs2=None, k=1, r=1):
    ...
    # index into the matrix to get the estiamtes for each source sample, drug pairs
    ...
    # index into the matrix to get the estiamtes for target sample, drug pairs
