...
import cross_val
import helpers
import model_helpers_jl as modeling

# Real values for K_LIST
...
def matrix_transfer(s, d, c, u_loc, v_loc, k, r):
    J = np.zeros((k, k))
...
def predict_transfer(model_seed, s_idx_src, d_idx_src, obs_src, s_idx_train, d_idx_train, obs_train, s_idx_test, d_idx_test, n_samp, n_drug, n_steps, k, r):
    ...
    for step in tqdm.trange(n_steps):
        svi.step(n_samp, n_drug, s_idx_src, d_idx_src, s_idx_train, d_idx_train, obs_src, len(obs_src), obs_train, len(obs_train), k=k, r=r)
        loss = svi.evaluate_loss(n_samp, n_drug, s_idx_src, d_idx_src, s_idx_train, d_idx_train, obs_src, len(obs_src), obs_train, len(obs_train), k=k, r=r)
        losses.append(loss)
    ...
    # need to retrive c, u, v and reconstruct s'!
    c_loc = pyro.param("AutoNormal.locs.c").detach().numpy()
    u_loc = []
    v_loc = []
    R = [i+1 for i in list(range(r))]
    for i in R:
    ...
    mat = matrix_transfer(s_loc, d_loc, c_loc, u_loc, v_loc, k, r)
...
def run_predict_transfer(model_seed, source_df, source_col, target_train_df, target_col, s_idx_test, d_idx_test, n_samp, n_drug, n_steps, k, r):
...
def predict_transfer_wrapper(source_df, source_col, target_train_df, target_col, s_idx_test, d_idx_test, n_samp, n_drug, n_steps, k, r):
    train_predict_list = []
    test_predict_list = []
...
def choose_k_r_transfer(method, target_train_df, target_col, split_type, n_samp, n_drug, n_steps, source_df=None, source_col=None):
    #print('Choose k via cross validation')
    ...
    for k in K_LIST:
        ...
        if max(avg_v) > optimal_v_corr:
            print('shiftting optimal parameters')
            optimal_v_corr = max(avg_v)
            optimal_r = ranks[np.argmax(avg_v)]
            optimal_k = k
            assert optimal_r != -np.inf and optimal_k != -np.inf and optimal_v_corr != -np.inf
    assert optimal_r <= optimal_k
    print('optimal k: ' + str(optimal_k), 'optimal r: ' + str(optimal_r))
    return optimal_k, optimal_r
...
def main():
    ...
    if method == 'raw':
        ...
        if method == 'target_only':
            ...
            source_df = helpers.get_source(data_fn, source_col)
            # cross validation to choose parameter k
            k, r = choose_k_r_transfer('transfer', target_train_df, target_col, split_type, n_samp, n_drug, n_steps, source_df, source_col)
            # given the above k, fit the model on the entire training set and make predictions
